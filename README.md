...
## Running onboardingAssesComplex in Quarkus dev mode
...
```
(onboardingAssesComplex should be included in the returned array)

Run the following to create an instance of the onboardingAssesComplex workflow:

```shell script
curl -XPOST -H "Content-Type: application/json" http://localhost:8080/onboardingAssesComplex -d@input.json
```
...
```
{
    ...
    "workflowdata": {
        "workflowOptions": {
            "currentVersion": {
                "id": "onboardingAssesComplex",
                "name": "onboardingAssesComplex"
            },
            "upgradeOptions": [],
            "migrationOptions": [],
            "newOptions": [],
